...
public record ModelRegistry(IList<PropertyCollection> Models, IList<InstanceClass> Singletons) : IProgramContext
{
...
public record Api(PropertyCollection Model, IProgramContext? Context = null) : ProgramModel<ClassModel>(Context ?? new ProgramContext())
{
    public override ClassModel Render()
    {
        ...
        var invocation = firstOrDefault.Invoke(Context.GetSingleton(dbContext), Literal(0));    // TODO: ID Lambda
