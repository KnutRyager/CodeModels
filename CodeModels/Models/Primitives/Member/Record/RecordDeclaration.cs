    ...
    IMember, IRecordDeclaration
{
    ...
    {
        var declaration = new RecordDeclarationImp(name, attributes?.ToAttributeListList() ?? AttributesList(), List(genericParameters), List(constraintClauses), List(baseTypeList), ListOfNoNull(members), @namespace, modifier ?? Modifier.Public);
