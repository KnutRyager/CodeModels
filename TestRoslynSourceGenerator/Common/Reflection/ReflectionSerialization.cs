...
public static class ReflectionSerialization
{
    ...
    public static string SerializeMethod(MethodInfo method)
    {
        ...
        if (method.IsGenericMethod || method.IsConstructedGenericMethod || method.ContainsGenericParameters)
            try
            {
    ...
    {
        var sb = new StringBuilder();
        var isGeneric = genericParameter?.ParameterType.IsGenericMethodParameter ?? false;
