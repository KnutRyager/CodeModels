...
public class MappingTableTests
{
    ...
    {
        var table = new ClassMappingTable<int, TableSpec>(new LazyLoadList<TableSpec>(() => FileUtil.ReadFileToType<TableSpec>("SampleForMappingTableParse.txt", ",")), x => x.TheValue);
    ...
    {
        var table = new MappingTable<int, string>(new LazyLoadList<(int, string)>(() => FileUtil.ReadFileToTuple<int, string>("SampleForMappingTableParse.txt", ",")));
